...
# Function to combine and save validation images
def save_combined_validation_images(epoch, val_images, val_reconstructed, output_dir="saved_combined_images"):
    """
    Save combined validation images (original + reconstructed) side-by-side for the entire batch.

    Args:
        epoch (int): Current epoch number.
        val_images (Tensor): Original validation images, shape [B, C, H, W].
        val_reconstructed (Tensor): Reconstructed validation images, shape [B, C, H, W].
        output_dir (str): Directory to save the combined images.
    """
    os.makedirs(output_dir, exist_ok=True)  # Ensure the output directory exists
    epoch_dir = os.path.join(output_dir, f"epoch_{epoch}")
    os.makedirs(epoch_dir, exist_ok=True)  # Subdirectory for the current epoch

    # Combine and save each image in the batch
    for idx in range(val_images.size(0)):
        original = val_images[idx].cpu()  # Original image
        reconstructed = val_reconstructed[idx].cpu()  # Reconstructed image

        # Combine the two images side-by-side
        combined_image = torch.cat((original, reconstructed), dim=2)  # Concatenate horizontally (width)

        # Save the combined image
        save_path = os.path.join(epoch_dir, f"val_combined_{idx + 1}.png")
        vutils.save_image(combined_image, save_path)

    print(f"Combined validation images saved in: {epoch_dir}")
